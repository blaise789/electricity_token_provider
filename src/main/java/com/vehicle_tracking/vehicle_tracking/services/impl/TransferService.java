    ...
    import com.vehicle_tracking.vehicle_tracking.repositories.TransferRepository;
    ...
    import com.vehicle_tracking.vehicle_tracking.services.standalone.MailService;
    ...
    //        check the old owner if already exists
        ...
        @Override
        @Transactional()
        public Page<TransferResponseDTO> getVehicleHistory(String chassisNumber, String plateNumber, Pageable pageable) {
            Vehicle vehicle ;
            ...
            Page<VehicleTransfer> vehicleTransfers = transferRepo.findByVehicle(vehicle, sortedPageable);
            // Map to DTOs and return
            TransferResponseDTO responseDTO = new TransferResponseDTO();
            return vehicleTransfers.map(vehicleTransfer -> {
                        responseDTO.setCurrentOwner(vehicleTransfer.getNewOwner().getNames());
                        responseDTO.setPreviousOwner(vehicleTransfer.getPreviousOwner().getNames());
                        responseDTO.setTransferDate(vehicleTransfer.getTransferDate());
                        return responseDTO;
                    }
            );
